...
function osmToGeoJSON(elements) {
  ...
  for (const el of elements) {
    ...
    if (aeroway === 'runway') {
      ...
      // Stands in OSM are usually short lines (the centerline of the parking spot)
      // or occasionally polygons. Convert lines to a midpoint Point feature so the
      // label appears in the right spot, and keep polygon coords intact.
      if (isClosed(el.geometry)) {
        ...
        features.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [mid.lon, mid.lat] }, properties: { ...props, kind: 'stand' } });
      }
